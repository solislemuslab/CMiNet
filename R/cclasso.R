...
#'
#' The main function to infer correlations for compositional data using Lasso.
#'
#' @param x A numeric matrix of dimension n x p, where n is the number of samples and p is the number of compositional variables.
#' @param counts Logical, indicating whether the compositional data matrix is a count matrix. Default is FALSE.
#' @param pseudo Numeric, the pseudo count added if `counts = TRUE`. Default is 0.5.
#' @param k_cv Integer, the number of folds for cross-validation. Default is 3.
#' @param lam_int Numeric vector of length 2, specifying the interval for the tuning parameter lambda. Default is `c(1e-4, 1)`.
#' @param k_max Integer, the maximum number of iterations for the golden section method. Default is 20.
#' @param n_boot Integer, the number of bootstrap replications. Default is 20.
#'
#' @return A list containing:
#' \item{var_w}{Variance estimation.}
#' \item{cor_w}{Correlation estimation.}
#' \item{p_vals}{P-values for elements of `cor_w` indicating if they are equal to 0.}
#' \item{lambda}{The final tuning parameter.}
#' \item{info_cv}{Information for cross-validation.}
#' @importFrom stats var pt

#' @export

cclasso <- function(x, counts, pseudo, k_cv,
                    lam_int, k_max, n_boot) {
  n <- nrow(x);
  p <- ncol(x);
  ...
  while(err > err_lam2 && k < k_max) {
    ...
    if (!is.na(fa2$cv_loss) && !is.na(fb2$cv_loss) && fa2$cv_loss > fb2$cv_loss) {
      a1 <- a2;
      a2 <- b2;
      fa2 <- fb2;
      b2 <- a1 + 0.618 * (b1 - a1);
    ...
    k <- k + 1;
    err <- b1 - a1;
    if (!is.na(fval_max) && !is.na(fval_min) && abs(fval_max - fval_min) / (1 + fval_min) <= err_fval) {
      break;
    }
  }
  info_cv <- list(lams = lams, fvals = fvals, k = k + 2,
    lam_int = 10^c(a1, b1));
  if ((!is.na(a1) && a1 == lam_int2[1]) || (!is.na(b1) && b1 == lam_int2[2])) {
...
boot_cclasso <- function(x, sigma, lambda2, n_boot = 20,
  ...
  for(k in 1:n_boot) {
    ind_samp <- sam_boot[, k];
    sigma2 <- cclasso_sub(sigma = sigma, vx = stats::var(x[ind_samp, ]),
  ...
  sigma_art <- cclasso_sub(sigma = sigma, vx = stats::var(sam_art),
    lambda2 = lambda2, wd = wd, u_f = u_f, u0_wd = u0_wd, d0_wd = d0_wd);
  ...
  p_vals <- stats::pt(cors2mod * sqrt((n - 2) / (1 - cors2mod^2)), df = n - 2);
