...
files1 <- list.files(pattern = "^Result_gut_.*\\.csv$")

df_list1 <- lapply(files1, function(file) {
  df <- read_csv(file, show_col_types = FALSE)
  parts <- str_match(file, "Result_gut_(.*?)_(\\d+)\\.csv")
  df$Group <- parts[2]
...
p <- ggplot(df_combined, aes(x = Method_Label, y = F_score, fill = Color)) +
  ...
  scale_y_continuous(limits = c(0.55, 1), expand = expansion(mult = c(0, 0.05)))+
...
# === Save the figure ===
ggsave("fscore_combined_final_gut.png", p_final, width = 14, height = 5, dpi = 300)
...
write.csv(summary_table, "network_performance_summary.csv", row.names = FALSE)
