...
load("weighted_networks_list_cor.RData")

# -------------------- LOAD & FILTER DATA --------------------
# Load your data
data <- read.csv("da_class_count_scabpit.csv")
dim(data)
# Filter samples
...
compute_binary_networks <- function(score) {
  ...
  result <- CMiNet(
    ...
    spiecEasi_mb = list(enabled = FALSE,params = spiecEasi_mb_params),
    spiecEasi_glasso = list(enabled = FALSE,params = spiecEasi_glasso_params),
    spring = list(enabled = FALSE,params = spring_params),
    gcoda = list(enabled =FALSE, params =gcoda_params),
    c_MI  = list(enabled =FALSE,params=c_MI_params),
  ...
  # Save results to CSV
  output_filename <- paste0("Result_soil_cor_", score, ".csv")
